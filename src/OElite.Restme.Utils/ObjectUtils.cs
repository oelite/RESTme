	...
	public static class ObjectUtils
	{
		...
		public static object GetPropertyValueEnhanced(this object obj, string propName)
		{
			...
			if (subObjIndex > 0)
			{
				...
				return objValue != null ? GetPropertyValueEnhanced(objValue, propName.Substring(subObjIndex + 1)) : null;
			}
			else
			...
			catch
			{
					...
					Nullable.GetUnderlyingType(propInfo.PropertyType) != null)
				{
					...
					try
					{
						...
						propInfo.SetValue(obj, StringUtils.GetStringValueOrEmpty(val).IsNullOrEmpty() ? null : enumValue,
							null);
					}
					catch
					{
