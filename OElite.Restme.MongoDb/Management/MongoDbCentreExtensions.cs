...
public static class MongoDbCentreExtensions
{
    ...
    /// <summary>
    /// Create optimized indexes for S3-style object storage collections
    /// Pre-configured for EdgeQ1 S3 storage scenarios
    /// </summary>
    /// <param name="dbCentre">The MongoDbCentre instance</param>
    /// <param name="options">S3 optimization options</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Bootstrap result</returns>
    public static async Task<DbManagementResult> BootstrapS3StorageAsync(
        this MongoDbCentre dbCentre,
        DbS3StorageOptions? options = null,
        CancellationToken cancellationToken = default)
    {
        options ??= new DbS3StorageOptions();
        var configuration = BuildS3StorageConfiguration(options);
    ...
    private static DbBootstrapConfiguration BuildS3StorageConfiguration(DbS3StorageOptions options)
    {
        var configuration = new DbBootstrapConfiguration
        {
            ...
            {
                // Objects collection (main storage)
                new()
                {
                ...
                // Uploads collection (multipart uploads)
                new()
                {
                    ...
                    Indexes = new List<DbIndexDefinition>
                    {
                        ...
                        new()
                        {
                            ...
                            TtlExpiration = TimeSpan.Zero
                        }
                    }
                },

                // Buckets collection (metadata)
                new()
                {
...
/// <summary>
/// S3 storage optimization options
/// </summary>
public class DbS3StorageOptions
{
    /// <summary>
    ...
    /// </summary>
    public string UploadsCollectionName { get; set; } = "uploads";

    /// <summary>
    /// Buckets collection name
    /// </summary>
    public string BucketsCollectionName { get; set; } = "buckets";
    ...
    /// <summary>
    /// Whether to enable pre-splitting
    /// </summary>
    public bool EnablePreSplitting { get; set; } = true;

    /// <summary>
    /// Number of pre-split chunks
    /// </summary>
    public int PreSplitCount { get; set; } = 256;

    /// <summary>
    /// TTL for objects (null for no expiration)
    /// </summary>
    public TimeSpan? ObjectTtl { get; set; }
}
